...
const Getgraph = () => {
  ...
  useEffect(() => {
    ...
    axios.get(API_URL)
      .then((res) => {
        ...
        const forecastData = Object.entries(dailyTemps).slice(0, 5).map(([date, temps], idx) => {
  ...
  return (
    <div className="weather-container" style={{ backgroundImage: bg ,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      ...
      <button onClick={() => navigate('/') } style={{backgroundColor:"blue", border:"none", padding:"10px",fontWeight:"666",borderRadius:"10px"}} >← Back</button>
    </div>
  );
};
